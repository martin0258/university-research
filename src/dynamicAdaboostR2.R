dynamicAdaboostR2 = function( file, windowLen = 4 ) {
  # Train/predict with AdaBoost.R2 for each period of each series.
  #
  # Args:
  ...
  {
    # form data
    wData <- windowing(data[, drama], windowLen)

    numEpisodes <- max(which(!is.na(data[, drama])))
    numCases <- numEpisodes - windowLen + 1
    for(trainEndIndex in 1:(numCases-1))
    {
      ...
      # One-step forecast
      xIndex <- 1:(windowLen-1)
      yIndex <- windowLen

      # Training phase (with error handling)
      model <- tryCatch({
        adaboostR2.train(wData[trainIndex, xIndex], 
                         wData[trainIndex, yIndex])
      }, error = function(err) {
        return(err)
      })
      ...
      # Testing phase
      prediction[testEpisode, drama] <- adaboostR2.predict(model, wData[testIndex, xIndex])
